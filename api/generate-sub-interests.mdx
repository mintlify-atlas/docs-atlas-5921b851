...
const subTopics = await generateSubInterests('Machine Learning');
// Returns: ["Neural Networks", "Deep Learning Frameworks", ...]

// In a client component (with transition)
const handleExpand = async (interest: string) => {
  const [isPending, startTransition] = useTransition();
  
  startTransition(async () => {
    const subInterests = await generateSubInterests(interest);
    // Process the results
  });
};
```
